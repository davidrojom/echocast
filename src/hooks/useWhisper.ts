...
export function useWhisper({ language, onTranscript, onError, enabled = true }: UseWhisperOptions) {
  ...
  useEffect(() => {
    ...
    if (!workerRef.current) {
      const blob = new Blob([whisperWorkerScript], { type: 'application/javascript' });
          ...
          case 'complete':
            ...
            if (data && typeof data.text === 'string') {
               ...
               if (text) {
                ...
                const text = data.map((item: { text: string }) => item.text).join(' ').trim();
                if (text) {
                    onTranscriptRef.current(text, true);
                }
            }
  ...
  const startListening = useCallback(async (deviceId?: string) => {
    ...
    try {
      ...
      if (onErrorRef.current) onErrorRef.current(err instanceof Error ? err : new Error(String(err)));
    }
  }, [isModelReady]);
    ...
    if (isListening) {
      interval = setInterval(() => {
        if (audioDataRef.current.length > 0) {
           ...
           const shouldProcess = (silenceDuration > SILENCE_DURATION) || (segmentDuration > MAX_SEGMENT_DURATION);
           ...
           if (shouldProcess && !isProcessingRef.current) {
             ...
             if (totalLength > 16000 * 0.2) {
                ...
                if (workerRef.current) {
                    ...
                    type: 'generate',
                    data: { audio: mergedAudio, language: language.split('-')[0] }
                    });
                }
             }
